...
const getRawGitUrl = (gitUrl) => {
  if (!gitUrl) return null;
  if (gitUrl.includes("github.com")) {
    return gitUrl.replace(/\.git$/, "").replace("github.com", "raw.githubusercontent.com").replace(/\/tree\//, "/") + "/main";
  }
...
const parseSchemaYaml = (yamlContent) => {
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    if (indentation === baseIndentation && trimmed.includes(":") && !trimmed.startsWith("type:") && !trimmed.startsWith("description:") && !trimmed.startsWith("default:") && !trimmed.startsWith("required:") && !trimmed.startsWith("options:")) {
      if (currentFieldName) {
        const field = {
        ...
        if (currentFieldDefault !== void 0) field.default = currentFieldDefault;
    ...
    } else if (trimmed.startsWith("description:") && indentation > baseIndentation) {
      const descMatch = trimmed.match(/description:\s*(.+)/);
      if (descMatch) {
        currentFieldDescription = descMatch[1].trim().replace(/^['"]|['"]$/g, "");
      }
      ...
      if (defaultMatch) {
        ...
        if (currentFieldType === "boolean" || defaultStr === "true" || defaultStr === "false") {
          currentFieldDefault = defaultStr === "true";
        } else if (currentFieldType === "int" || currentFieldType === "integer" || currentFieldType === "number") {
          currentFieldDefault = parseInt(defaultStr, 10);
        } else {
          currentFieldDefault = defaultStr;
        }
      }
    } else if (trimmed.startsWith("required:") && indentation > baseIndentation) {
      ...
      for (let j = i + 1; j < lines.length; j++) {
        ...
        if (optTrimmed.startsWith("-")) {
          const option = optTrimmed.substring(1).trim().replace(/^['"]|['"]$/g, "");
...
const buildInitialFormData = (metadata) => {
  const defaults = {};
...
const fetchPlateMetadata = async (gitUrl) => {
  try {
    ...
    const metadata = { raw: yamlContent };
    const lines = yamlContent.split("\n");
    let currentSection = "";
    let schemaYaml = "";
    let filesYaml = "";
    ...
    for (let i = 0; i < lines.length; i++) {
      ...
      if (inSchema && trimmed && !trimmed.startsWith("schema:")) {
        ...
        if (lineIndentation > schemaIndentation || !trimmed) {
          schemaYaml += "\n" + line;
        } else if (trimmed && lineIndentation <= schemaIndentation && !trimmed.startsWith("#")) {
          inSchema = false;
        }
        ...
        if (lineIndentation > modulesIndentation || !trimmed) {
          modulesYaml += "\n" + line;
        } else if (trimmed && lineIndentation <= modulesIndentation && !trimmed.startsWith("#")) {
          inModules = false;
        }
        ...
        if (lineIndentation > filesIndentation || !trimmed) {
          filesYaml += "\n" + line;
        } else if (trimmed && lineIndentation <= filesIndentation && !trimmed.startsWith("#")) {
          inFiles = false;
        }
      } else if (currentSection === "readme" && trimmed && !trimmed.startsWith("structure:") && !trimmed.startsWith("schema:") && !trimmed.startsWith("files:") && !trimmed.startsWith("modules:")) {
        readmeContent += "\n" + trimmed;
      } else if (currentSection === "structure" && trimmed && !trimmed.startsWith("readme:") && !trimmed.startsWith("schema:") && !trimmed.startsWith("files:") && !trimmed.startsWith("modules:")) {
        structure += "\n" + trimmed;
      }
    }
    ...
    if (modulesYaml.trim()) {
      ...
      for (const modLine of moduleLines) {
        ...
        if (moduleMatch && modLine.search(/\S/) === modulesIndentation) {
          ...
          if (enabledMatch) {
            modulesObj[currentModuleName].enabled = enabledMatch[1].trim() === "true";
          }
        }
      }
...
export { buildInitialFormData, fetchPlateMetadata, formatDate };
//# sourceMappingURL=plateMetadata.esm.js.map
